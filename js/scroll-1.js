...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Fade-IN only: the copy ramps up and is fully in well before the phase
  // ends, then stays at full opacity through the LAST frame of scroll-1 (no
  // fade-out). This guarantees the copy is complete on the final frame.
  ...
  function render(progress) {
    ...
    // scrub. On desktop it travels from the right column to the left.
  ...
  mm.add("(max-width: 768px)", () => {
    // Mobile scrubs exactly like desktop (the main.js pin drives it via
    // __scroll1Render); render() keeps the card centred (x:0) on mobile.
    window.__scroll1Render = render;
    render(0);
